...
bool USBDevice::writeData(std::vector<uint8_t> data, const std::string& device) {
    if (hidDevice < 0 || !connected || data.empty()) {
        if (!device.empty()) {
            debug("[%s] HID device not open, not connected, or empty data\n", device.c_str());
        } else {
    ...
    if (bytesWritten == (ssize_t)data.size()) {
        if (!device.empty()) {
            debug_force("[%s] Write successful via raw write()\n", device.c_str());
        } else {
            debug_force("[Unknow] Write successful via raw write()\n");
        }
        return true;
    }

    if (!device.empty()) {
        debug_force("[%s] Raw write failed: %s (wrote %zd of %zu bytes)\n",
                    device.c_str(), strerror(errno), bytesWritten, data.size());
    } else {
        debug_force("[Unknown] Raw write failed: %s (wrote %zd of %zu bytes)\n",
